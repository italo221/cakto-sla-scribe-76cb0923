...
export default function GlobalTicketButton() {
  ...
  const handleCreateTicket = () => {
    navigate('/', { state: { action: 'create-ticket' } });
  };

  return (
    <Button
      onClick={handleCreateTicket}
      className="fixed bottom-6 right-6 z-50 h-14 w-14 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 bg-primary hover:bg-primary/90 border-0"
      size="icon"
    >
