-- Criar view para lista de tickets do time
CREATE OR REPLACE VIEW team_ticket_list_v AS
SELECT 
  t.id,
  t.ticket_number,
  t.titulo,
  ...
  t.data_criacao,
  t.setor_id,
  t.responsavel_interno,
  t.prazo_interno as due_at,
  t.tags,
  -- Campos calculados
  t.data_criacao::timestamp as created_at,
  CASE 
    WHEN t.status = 'resolvido' OR t.status = 'fechado' THEN t.updated_at
    ELSE NULL 
  END as resolved_at,
  -- Campo para primeira mudança para "em_andamento" (será calculado via logs)
  NULL::timestamp as first_in_progress_at,
  -- Campo de ordem manual (pode ser adicionado futuramente)
  0 as order_index,
  -- Verificar se está vencido
  CASE 
    WHEN t.prazo_interno IS NOT NULL AND t.status NOT IN ('resolvido', 'fechado') 
    THEN t.prazo_interno < NOW()
    ELSE false 
  END as is_overdue,
  -- Idade em dias
  EXTRACT(EPOCH FROM (NOW() - t.data_criacao))/86400 as age_days
FROM sla_demandas t
WHERE t.setor_id IS NOT NULL;

-- Aplicar RLS na view
ALTER VIEW team_ticket_list_v SET (security_barrier = true);

-- Política RLS para a view
DROP POLICY IF EXISTS "team_ticket_list_v_select_policy" ON team_ticket_list_v;
CREATE POLICY "team_ticket_list_v_select_policy" ON team_ticket_list_v
FOR SELECT 
USING (
  -- Super admin pode ver tudo
  is_super_admin() OR 
  -- Usuários podem ver tickets dos seus setores
  setor_id IN (
    SELECT us.setor_id 
