...
-- Add policy to prevent users from updating their own user_type
CREATE POLICY "profiles_prevent_self_role_change" ON public.profiles
  FOR UPDATE USING (
    -- Users can update their own profile but not their user_type
    auth.uid() = user_id AND (
      OLD.user_type = NEW.user_type OR is_admin()
    )
  );

-- Remove the old update policy and replace with the secure one
DROP POLICY IF EXISTS "profiles_update_own" ON public.profiles;

-- Allow users to update their own profile (except user_type unless admin)
