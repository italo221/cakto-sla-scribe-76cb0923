-- Fix the pin_ticket function to call user_has_setor_access with correct parameters
...
BEGIN
  ...
  IF NOT (user_has_setor_access(p_team_id, current_user_id) OR is_admin()) THEN
...
BEGIN
  -- Verificar se usuário tem permissão
  IF NOT (user_has_setor_access(p_team_id, auth.uid()) OR is_admin()) THEN
