-- Definitive mention_search to list all users for @mentions (no hidden filters)
...
begin
  ...
  if q is null or btrim(q) = '' then
    return query
      select p.user_id,
             coalesce(p.nome_completo, p.email) as nome_completo,
             p.email
...
-- Lock down and grant to authenticated role only
